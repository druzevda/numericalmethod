...
void simpleIteration(const std::vector<func>& f, const std::vector<func>& phi, std::vector<double>& xnp1, const double eps) {
    std::vector<double> xn = xnp1;
    while (std::abs(calcMaxDelta(f, xnp1)) > eps) {
        xn = xnp1;
